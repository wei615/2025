...
function draw() {
  ...
  // 計算長方形的位置與大小
  let rectWidth = windowWidth / 2; // 長方形寬度為視窗寬度的一半
  let rectHeight = windowHeight / 2; // 長方形高度為視窗高度的一半
  ...
  if (currentQuestion < questions.length) {
    if (!isAnswered) {
      // 顯示題目
      textSize(40); // 題目文字大小為 40px
      text(questions[currentQuestion].question, windowWidth / 2, rectY + 70);

      // 顯示✕和◯選項
      textSize(80); // 選項文字大小為 80px
      text("✕", rectX + rectWidth / 3, rectY + rectHeight - 110); // 左邊為✕，往中間集中
      text("◯", rectX + (rectWidth * 2.8) / 4, rectY + rectHeight - 110); // 右邊為◯，往中間集中
    } else if (!showExplanation) {
      // 顯示「答錯了！」或「答對了！」文字
      textSize(40); // 解答文字大小為 40px
      text(
        isCorrect ? "答對了！" : "答錯了！",
        windowWidth / 2,
        rectY + rectHeight / 2
...
function mousePressed() {
  ...
  if (currentQuestion < questions.length) {
    if (!isAnswered) {
      ...
      let xOptionY = rectY + rectHeight - 110; // ✕ 的 Y 座標
      if (dist(mouseX, mouseY, xOptionX, xOptionY) < 40) { // 點擊範圍半徑設為 40
        isAnswered = true;
      ...
      let oOptionY = rectY + rectHeight - 110; // ◯ 的 Y 座標
      if (dist(mouseX, mouseY, oOptionX, oOptionY) < 40) { // 點擊範圍半徑設為 40
        isAnswered = true;
